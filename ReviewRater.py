    ...
    with nlp.disable_pipes(training_excluded_pipes):
        ...
        for i in range(iterations):
            ...
            with textcat.model.use_params(optimizer.averages):
                ...
                print(str(loss['textcat']) + '\t' + str(evaluation_results['precision']) + '\t' + str(evaluation_results['recall']) + '\t' + str(evaluation_results['f-score']))
...
def test_model():
    ...
    print("\nCombined: " + str(precision) + "\t" + str(recall) + "\t" + str(fScore))
...
def test_model_single():
    ...
    print("----\nPredicted Rating: " + str(prediction) + "\tScore: " + str(score) + "\n----")
            ...
            if review.endswith(".txt"):
                with open(f"{labeled_directory}/{review}", encoding="utf8") as f:
                    ...
                    if stopwordlessText.strip():
                        spacy_label = {"cats": {"one": "one" == label, "two": "two" == label, "three": "three" == label, "four": "four" == label, "five": "five" == label}}
